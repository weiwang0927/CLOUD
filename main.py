...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', type=str, default='./save_model/beauty/', help='save_path')
    parser.add_argument('-m', type=str, default="train", help='train_valid_test')          # train valid  test
    parser.add_argument('-r', action='store_true', help='resume')
    parser.add_argument('-n', type=int, default=12104, help='item_num')  # Beauty dataset contains 12101 items, plus padding, EOS, mask, yelp 16555, sports 18360
    ...
    if mode == 'train':
        ...
        if resume:
            ...
            model.load_state_dict(torch.load(save_path + 'model/CLOUD-' + str(last_epoch) + '.pth'))
        else:
        ...
        while epoch < epochs:
            ...
            for batch in dataloader:
                step += 1                        
                seq, masked_seq, random_modified_seqs, l1_ground_truth, l2_ground_truth, insert_seqs, rec_loss_mask, sim_seqs, candicate_items, zero_mark = batch
                if torch.cuda.is_available():
                    seq = seq.cuda()
                    ...
                    sim_seqs = sim_seqs.cuda()    # batch_size * k * seq_len
                    candicate_items = candicate_items.cuda()
                    zero_mark = zero_mark.cuda()
                ...
                full_layer_output, insert_net_output, padding_mask, par_list = model.corrector_forward(
                    random_modified_seqs, insert_seqs, sim_seqs, candicate_items, zero_mark)
                ...
                with torch.no_grad():
                    ...
                    full_layer_output_ori, insert_net_output_ori = model.corrector_inference(seq, sim_seqs, candicate_items, zero_mark)
            ...
            torch.save(model.state_dict(), save_path + 'model/CLOUD-' + str(epoch) + '.pth')
            print(
    ...
    if mode == "valid":
        ...
        while epoch <= epochs:
            ...
            model.load_state_dict(torch.load(save_path + 'model/CLOUD-' + str(epoch) + '.pth'))
            with torch.no_grad():
                for batch in dataloader:
                    step += 1
                    _, valid_neg, masked_seq, leave_one_seq, target, sim_seqs, candicate_items, zero_mark = batch
                    if torch.cuda.is_available():
                        valid_neg = valid_neg.cuda()
                        masked_seq = masked_seq.cuda()
                        leave_one_seq = leave_one_seq.cuda()
                        target = target.cuda()
                        sim_seqs = sim_seqs.cuda()
                        candicate_items = candicate_items.cuda()
                        zero_mark = zero_mark.cuda()
                    ...
                    # corrected sequence
                    full_layer_output_ori, insert_net_output_ori = model.corrector_inference(leave_one_seq, sim_seqs, candicate_items, zero_mark)
            ...
            total_result_dict_modified['ndcg@5'] = get_metrics('ndcg@5', total_result_modified)
            total_result_dict_modified['ndcg@10'] = get_metrics('ndcg@10', total_result_modified)
            total_result_dict_modified['ndcg@20'] = get_metrics('ndcg@20', total_result_modified)
            total_result_dict_modified['sum'] = total_result_dict_modified['recall@5'] + total_result_dict_modified[
                ...
                total_result_dict_modified['mrr@10'] + total_result_dict_modified['mrr@20'] + \
                                                total_result_dict_modified['ndcg@5'] + total_result_dict_modified['ndcg@10'] + total_result_dict_modified['ndcg@20']
    ...
    if mode == "test":
        ...
        while epoch <= epochs:
            ...
            with torch.no_grad():
                for batch in dataloader:
                    step += 1
                    _, test_neg, neg_num, masked_seq, leave_one_seq, target, sim_seqs, candicate_items, zero_mark, real_len = batch
                    if torch.cuda.is_available():
                        ...
                        real_len = real_len.cuda()

                    # original sequence
                    recommender_output = model.forward(masked_seq)
                    recommender_output = recommender_output[masked_seq == item_num - 1]
                    result = evaluate_function(recommender_output, target, test_neg, neg_num)
                    total_result.extend(result)
                    # corrected sequence
                    full_layer_output_ori, insert_net_output_ori = model.corrector_inference(leave_one_seq, sim_seqs, candicate_items, zero_mark)
                    modified_leave_one_seqs = model.seqs_correction(full_layer_output_ori, insert_net_output_ori,leave_one_seq)
                    ...
                    recommender_modified_output = model.forward(modified_masked_seqs)
                    recommender_modified_output = recommender_modified_output[modified_masked_seqs == item_num - 1]
                    result_modified = evaluate_function(recommender_modified_output, target, test_neg, neg_num)
                    total_result_modified.extend(result_modified)
                    similarity, jac_sim = cal_similarity(masked_seq, modified_masked_seqs, real_len)
                    similarity_list.extend(similarity)
                    jac_sim_list.extend(jac_sim)
            ...
            similarity_result = {'similarity': sum(similarity_list) / len(similarity_list),
                                 'jac_sim': sum(jac_sim_list) / len(jac_sim_list),
                                 'operation': operation_list.tolist()}
